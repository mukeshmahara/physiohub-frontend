  ...
  <div className="flex flex-col gap-4 p-5 transition hover:bg-slate-50 sm:flex-row sm:items-center sm:p-6">
    ...
    <div className="grid flex-1 grid-cols-2 gap-3 text-sm sm:grid-cols-3">
      <span className="flex items-center gap-2 text-slate-600">
        ...
        {appointment.time}{" "}
        <span className="text-xs text-slate-400">({appointment.duration})</span>
      </span>
      <span className="flex items-center gap-2 text-slate-600">
    ...
    <div className="flex items-center justify-between gap-3 sm:w-32 sm:justify-end">
      <span
        className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusStyles[appointment.status] || "bg-slate-100 text-slate-600"}`}
      >
        {appointment.status}
      </span>
      <button
        ...
        aria-label={`More options for ${appointment.patient}`}
      >
